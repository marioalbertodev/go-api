...
import (
	...
	"github.com/marioalbertodev/go-postgres-microblog/pkg/user"
)
...
func (ur *UserRepository) Create(ctx context.Context, u *user.User) error {
	...
	if err := u.HashPasword(); err != nil {
		return err
	}

	row := ur.Data.DB.QueryRowContext(ctx, q, u.FirstName, u.LastName, u.Username,
		u.Email, u.Picture, u.PasswordHash, time.Now(), time.Now())
...
func (ur *UserRepository) Update(ctx context.Context, id uint, u user.User) error {
	...
	_, err = stmt.ExecContext(
		ctx, u.FirstName, u.LastName, u.Email, u.Picture, time.Now(), id,
	)
	if err != nil {
		return err
	}
